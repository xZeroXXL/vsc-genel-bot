...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const botRole = interaction.guild.members.cache.get(client.user.id).roles
      .highest;
    ...
    if (targetRole.position >= botRole.position) {
      const embed = new EmbedBuilder()
        .setColor("Red")
        .setDescription(
          "❌ | Bunu yapmak için rolüm yetersiz, rolümü daha yukarı taşır mısın 👉👈 🥺"
        );
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor("Random")
        .setDescription(
          `✅ | Başarıyla ${user} Kullanıcısının ${rol.name} Rolü Alındı!`
        );
      ...
      const embed = new EmbedBuilder()
        .setColor("Red")
        .setDescription(
          "❌ | Rol çıkarma sırasında bir hata oluştu. Lütfen daha sonra tekrar deneyin."
        );
