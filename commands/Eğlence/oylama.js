...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const filter = (i) =>
      i.customId === "oylamaModal" && i.user.id === interaction.user.id;
    interaction
      .awaitModalSubmit({ filter, time: 60000 })
      .then(async (modalInteraction) => {
        const oylamaMetni =
          modalInteraction.fields.getTextInputValue("oylamaMetni");
        ...
        const collector = msg.createMessageComponentCollector({
          filter: (i) => i.customId.endsWith("oylama"),
          time: süreMs,
        });
      ...
      .catch((err) => {
        console.error(
          "Modal gönderimi zaman aşımına uğradı veya başarısız oldu:",
          err
        );
        interaction.followUp({
          content:
            "❌ | Mesaj yazma işlemi sırasında bir hata oluştu veya işlem zaman aşımına uğradı.",
          ephemeral: true,
        });
      });
  },
